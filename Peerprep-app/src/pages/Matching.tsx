...
export const Matching: React.FC = () => {
  ...
  const attemptMatch = async () => {
    try {
      const response = await fetch(
        "http://localhost:3003/matching/attempt",
        {
          method: "POST",
        },
      );
      ...
      console.error(
        "Failed to reach matching service for match attempt:",
        err,
      );
    }
  };
  ...
  const loadRequestStatus = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3003/matching/requests/${state.requestId}`,
      );
      ...
      try {
        ...
        console.error(
          "Non-JSON response from matching service (status):",
          text,
        );
      }
      ...
      if (data?.matchRequest) {
        ...
        if (data.matchRequest.status === "MATCHED") {
          navigate("/workspace", {
            state: {
              difficulty:
                data.match?.requesterDifficulty ??
                data.matchRequest.difficulty,
      ...
      setRequestError(
        "Unable to reach matching service for status. Please try again.",
      );
    } finally {
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      try {
        ...
        console.error(
          "Non-JSON response from matching service (cancel):",
          text,
        );
      }
      ...
      console.error(
        "Failed to reach matching service for cancellation:",
        err,
      );
      setRequestError(
        "Unable to reach matching service for cancellation. Please try again.",
      );
    } finally {
        ...
        <div className="matching-card-wrapper">
          <Card className="matching-card">
            <div className="matching-header">
              <Users className="matching-icon" />
              <h1 className="matching-title">Finding a Peer...</h1>
            </div>
            ...
            <div className="matching-details">
              ...
              {state?.timeAvailableMinutes && (
                <div className="detail-item mt-2">
                  ...
                  <span className="detail-value tag">
                    {state.timeAvailableMinutes} minutes
                  </span>
                </div>
              )}
            </div>
